 ...
 * @subpackage MoceanAPI Abandoned Carts/public
 * @author     Micro Ocean Technologies
 */
 
class MoceanAPI_Public{
	...
	public function enqueue_styles(){
		if($this->exit_intent_enabled()){ //If Exit Intent Enabled
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/moceanapi-abandoned-carts-public.css', array(), $this->version, 'all' );
		}
	}
	...
	public function enqueue_scripts(){
		if($this->exit_intent_enabled()){ //If Exit Intent Enabled
			...
			if(get_option('moceanapi_abandoned_carts_exit_intent_test_mode')){ //If Exit Intent Test mode is on
				$data = array(
			...
			wp_enqueue_script( $this->plugin_name . 'exit_intent', plugin_dir_url( __FILE__ ) . 'js/moceanapi-abandoned-carts-public-exit-intent.js', array( 'jquery' ), $this->version, false );
	...
	function add_additional_scripts_on_checkout(){
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/moceanapi-abandoned-carts-public.js', array( 'jquery' ), $this->version, false );
	...
	function save_cart(){
		...
		if(get_option('moceanapi_abandoned_carts_exclude_ghost_carts') && $ghost){ //If Ghost carts are disabled and current cart is a ghost cart - do not save it
			return;
		}
	...
	function create_new_cart( $ghost ){
		...
		$this->set_moceanapi_abandoned_carts_session($cart['session_id']);
	}
	...
	function get_user_data(){
		...
		if ( is_user_logged_in() && !isset( $_POST["action"] )){ //If user has signed in and the request is not triggered by checkout fields or Exit Intent
			...
			$other_fields = array(
				...
				'moceanapi_abandoned_carts_billing_address_1' 		=> sanitize_text_field($address_1),
				'moceanapi_abandoned_carts_billing_address_2' 		=> sanitize_text_field($address_2),
				'moceanapi_abandoned_carts_billing_state' 			=> sanitize_text_field($state),
				'moceanapi_abandoned_carts_shipping_first_name' 	=> sanitize_text_field($shipping_name),
				'moceanapi_abandoned_carts_shipping_last_name' 		=> sanitize_text_field($shipping_surname),
				'moceanapi_abandoned_carts_shipping_company' 		=> sanitize_text_field($shipping_company),
				'moceanapi_abandoned_carts_shipping_country' 		=> sanitize_text_field($shipping_country),
				'moceanapi_abandoned_carts_shipping_address_1' 		=> sanitize_text_field($shipping_address_1),
				'moceanapi_abandoned_carts_shipping_address_2' 		=> sanitize_text_field($shipping_address_2),
				'moceanapi_abandoned_carts_shipping_city' 			=> sanitize_text_field($shipping_city),
				'moceanapi_abandoned_carts_shipping_state' 			=> sanitize_text_field($shipping_state),
				'moceanapi_abandoned_carts_shipping_postcode' 		=> $shipping_postcode,
				'moceanapi_abandoned_carts_order_comments' 			=> sanitize_text_field($comments),
				'moceanapi_abandoned_carts_create_account' 			=> $create_account,
				'moceanapi_abandoned_carts_ship_elsewhere' 			=> $ship_elsewhere
			);
	...
	/**
	 * Method sets MoceanAPI session id value
	 *
	 *  
	 */
	function set_moceanapi_abandoned_carts_session($session_id){
		if(!WC()->session->get('moceanapi_abandoned_carts_session_id')){ //In case browser session is not set, we make sure it gets set
			WC()->session->set('moceanapi_abandoned_carts_session_id', $session_id); //Storing session_id in WooCommerce session
		}
	}
	...
	function update_logged_customer_id(){
		if(is_user_logged_in()){ //If a user is logged in
			...
			if( WC()->session->get('moceanapi_abandoned_carts_session_id') !== NULL && WC()->session->get('moceanapi_abandoned_carts_session_id') !== $customer_id){ //If session is set and it is different from the one that currently is assigned to the customer
				...
				$wpdb->prepare('%s',
					$wpdb->update(
						...
						array('session_id' => WC()->session->get('moceanapi_abandoned_carts_session_id'))
					)
				);

				WC()->session->set('moceanapi_abandoned_carts_session_id', $customer_id);
				$cart = $this->read_cart();
				$user_data = $this->get_user_data();
	...
	public function restore_input_data( $fields = array() ) {
		...
		if($row){ //If we have a user with such session ID in the database
			...
			if($other_fields){
				(empty( $_POST['billing_company'])) ? $_POST['billing_company'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_billing_company'])) : '';
				(empty( $_POST['billing_address_1'])) ? $_POST['billing_address_1'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_billing_address_1'])) : '';
				(empty( $_POST['billing_address_2'])) ? $_POST['billing_address_2'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_billing_address_2'])) : '';
				(empty( $_POST['billing_state'])) ? $_POST['billing_state'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_billing_state'])) : '';
				(empty( $_POST['shipping_first_name'])) ? $_POST['shipping_first_name'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_first_name'])) : '';
				(empty( $_POST['shipping_last_name'])) ? $_POST['shipping_last_name'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_last_name'])) : '';
				(empty( $_POST['shipping_company'])) ? $_POST['shipping_company'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_company'])) : '';
				(empty( $_POST['shipping_country'])) ? $_POST['shipping_country'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_country'])) : '';
				(empty( $_POST['shipping_address_1'])) ? $_POST['shipping_address_1'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_address_1'])) : '';
				(empty( $_POST['shipping_address_2'])) ? $_POST['shipping_address_2'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_address_2'])) : '';
				(empty( $_POST['shipping_city'])) ? $_POST['shipping_city'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_city'])) : '';
				(empty( $_POST['shipping_state'])) ? $_POST['shipping_state'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_shipping_state'])) : '';
				(empty( $_POST['order_comments'])) ? $_POST['order_comments'] = sprintf('%s', esc_html($other_fields['moceanapi_abandoned_carts_order_comments'])) : '';
			}

			//Checking if Create account should be checked or not
			if(isset($other_fields['moceanapi_abandoned_carts_create_account'])){
				if($other_fields['moceanapi_abandoned_carts_create_account']){
					add_filter( 'woocommerce_create_account_default_checked', '__return_true' );
				}
			}

			//Checking if Ship to a different location must be checked or not
			if(isset($other_fields['moceanapi_abandoned_carts_ship_elsewhere'])){
				if($other_fields['moceanapi_abandoned_carts_ship_elsewhere']){
	...
	function increase_recoverable_cart_count(){
		update_option('moceanapi_abandoned_carts_recoverable_cart_count', get_option('moceanapi_abandoned_carts_recoverable_cart_count') + 1);
	}
	...
	function increase_ghost_cart_count(){
		update_option('moceanapi_abandoned_carts_ghost_cart_count', get_option('moceanapi_abandoned_carts_ghost_cart_count') + 1);
	}
	...
	function decrease_recoverable_cart_count( $count ){
		update_option('moceanapi_abandoned_carts_recoverable_cart_count', get_option('moceanapi_abandoned_carts_recoverable_cart_count') - $count);
	...
	function decrease_ghost_cart_count( $count ){
		update_option('moceanapi_abandoned_carts_ghost_cart_count', get_option('moceanapi_abandoned_carts_ghost_cart_count') - $count);
	}
	...
	function display_exit_intent_form(){
		...
		if( WC()->cart->get_cart_contents_count() > 0 ){ //If the cart is not empty
			...
			if (isset( $_POST["moceanapi_abandoned_carts_insert"])) { //In case function triggered using Ajax Add to Cart
	...
	function build_exit_intent_output( $current_user_is_admin ){
		...
		if (isset( $_POST["moceanapi_abandoned_carts_insert"])){ 
			$output = $this->get_template( 'moceanapi-abandoned-carts-exit-intent.php', array('main_color' => $main_color, 'inverse_color' => $inverse_color));
			die();
		}else{
			return $this->get_template( 'moceanapi-abandoned-carts-exit-intent.php', array('main_color' => $main_color, 'inverse_color' => $inverse_color));
		}
	}
	...
	function exit_intent_enabled(){
		$exit_intent_on = get_option('moceanapi_abandoned_carts_exit_intent_status');
		$test_mode_on = get_option('moceanapi_abandoned_carts_exit_intent_test_mode');
		$current_user_is_admin = current_user_can( 'manage_options' );
	 ...
	 *
	 * Search Order:
	 * 1. /themes/theme/moceanapi-abandoned-carts-templates/$template_name
